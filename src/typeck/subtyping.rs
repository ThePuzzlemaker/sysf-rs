...
pub fn subtype(ctx: &mut TyCtxt, ty1: &Ty, ty2: &Ty) -> Option<()> {
    ...
    match (ty1, ty2) {
        ...
        // <:Exvar
        (Ty::ExstVar(idx1), Ty::ExstVar(idx2))
            if idx1 == idx2 && ctx.contains_unsolved_evar(*idx1) => {}
        // <:->
        ...
        (Ty::Forall(a), b) => {
            ...
            let a = a.clone().subst_uvar0(&Ty::ExstVar(evar));
            subtype(ctx, &a, b)?;
...
pub fn inst_left(ctx: &mut TyCtxt, evar: usize, ty: &Ty) -> Option<()> {
    ...
    if !ctx.contains_unsolved_evar(evar) {
    ...
    match ty {
        ...
        // InstLReach
        Ty::ExstVar(beta) => {
            if !ctx.contains_unsolved_evar(*beta) {
...
pub fn inst_right(ctx: &mut TyCtxt, ty: &Ty, evar: usize) -> Option<()> {
    ...
    match ty {
        ...
        Ty::Forall(body) => {
            ...
            let body = body.clone().subst_uvar0(&Ty::ExstVar(beta));
